...
class Net(nn.Module):
    ...
    def forward(self, x): # 非residual
        ...
        # 第一次交叉
        x_up = self.att_up1(x_up)
        x_up1 = self.up_layer2_1(x_up)
        x_up1 = self.up_layer2_2(x_up1)

        x_down = self.att_down1(x_down)
        x_down1 = self.down_layer2_1(x_down)
        x_down1 = self.down_layer2_2(x_down1)
        # x_down1 = self.down_layer2_3(x_down1)

        # 第二次交叉
        x_up1 = self.att_up2(x_up1)
        x_up2 = self.up_layer3_1(x_up1)
        x_up2 = self.up_layer3_2(x_up2)

        x_down1 = self.att_down2(x_down1)
        x_down2 = self.down_layer3_1(x_down1)
